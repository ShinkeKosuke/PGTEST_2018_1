...
try{
    ...
    if(count($twitterPictures) > 0){
        foreach($twitterPictures as $twitterPicture){
            $data = file_get_contents($twitterPicture);
            $fileName = basename($twitterPicture);
            file_put_contents($directory_path.'/'.$fileName, $data);
        }
     }else {
       echo "対象の画像が存在しませんでした。";
     }
...
function getTwitterInfo($count){
    ...
    curl_setopt( $curl , CURLOPT_TIMEOUT , 30 ) ;    // タイムアウトの秒数
    ...
    if(isset($obj->errors)){
        throw new Exception($obj->errors[0]->message);
    }
